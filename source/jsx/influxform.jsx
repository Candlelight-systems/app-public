...
class AppForm extends React.Component {
  ...
  validateConfig() {
    this.props.onValidate(this.cleanState(this.state));
  }
  ...
  async createDB() {
    ...
    let address = `http://${this.state.host}:${
      this.state.port
    }/query?u=${u}&p=${p}`;

    try {
      const reply = await postJSON(
        address + `&q=${encodeURIComponent(`CREATE DATABASE "${db}"`)}`
      );

      const reply_privileges = await postJSON(
        address + `&q=${encodeURIComponent(`GRANT ALL ON "${db}" TO ${u}`)}`
      );
  ...
  async verifyConfig() {
    ...
    try {
      if (
        this.state.host == 'localhost' ||
        this.state.host.slice(0, 3) == '127'
      ) {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.formState !== nextProps.formState) {
      this.setState(this.cleanState(nextProps.formState));
    }
  }

  componentDidMount() {
    this.setState(this.cleanState(this.props.formState));
  }

  cleanState(state) {
    if (state.db_authentication) {
      delete state.db_authentication;
    }
    ...
    if (state.db_exists) {
      delete state.db_exists;
    }

    if (state.db_dbexists) {
      delete state.db_dbexists;
    }
  ...
  render() {
    ...
    switch (this.state.db_connection) {
      ...
      case 'error':
        ...
        status_text_db_connection = (
          <span>Could not connect to the database</span>
        );
        break;
    ...
    switch (this.state.db_authentication) {
      ...
      case 'No user defined':
        ...
        status_text_db_auth = (
          <span>
            No user was defined. If no authentication to the DB is required,
    ...
    switch (this.state.db_dbexists) {
      ...
      default:
        ...
        status_text_db_dbexists = (
          <span>
            {this.state.db_dbexists}
            <button className="btn btn-default" onClick={this.createDB}>
              Create
            </button>
          </span>
        );
        ...
        case 'progress':
          status_progress = (
            <div className="alert alert-info">
              Uploading to {this.props.uploading.host} in progress
            </div>
          );
        ...
        case 'error':
          status_progress = (
            <div className="alert alert-danger">
              Error while uploading to {this.props.uploading.host}. Check that
              the host is running
            </div>
          );
    ...
    return (
      <div className="container-fluid">
        ...
        {status_text_db_connection && (
          <div className={'alert ' + status_class_db_connection}>
            {status_text_db_connection}
          </div>
        )}
        {status_text_db_auth && (
          <div className={'alert ' + status_class_db_auth}>
            {status_text_db_auth}
          </div>
        )}
        {status_text_db_dbexists && (
          <div className={'alert ' + status_class_db_dbexists}>
            {status_text_db_dbexists}
          </div>
        )}
        ...
        <form onSubmit={this.submit} className="form-horizontal">
          ...
          <div className="alert alert-info">
            If the authentication is disabled in influxDB, credentials are not
            checked. Read{' '}
