...
class ScheduleLight extends React.Component {
  ...
  async apply() {
    let saveJSON = {
      ...
      control: {
        setPoint: this.state.fixed_intensity
          ? parseFloat(this.state.fixed_intensity_val)
          : false,
        scheduling: {
          enable: !this.state.fixed_intensity,
          basis: this.state.schedule_basis,
          intensities: this.state.schedule_values
            .split('\n')
    ...
    if (this.state.uv) {
      saveJSON.control.uv = {
        setPoint: parseFloat(this.state.fixed_uv_intensity_val) / 100
      };
    }
    ...
    this.setState({
      message:
        'Saving light controllers and setting new light intensity. This may take a while...',
      error: false,
      success: false
    });

    return fetch(
      `http://${this.props.config.trackerHost}:${this.props.config.trackerPort}/lightSetControl`,
      {
        method: 'POST',
        headers: headers,
        body: body
  ...
  async componentDidMount() {
    ...
    await fetch(
      ...
      .catch(error => {
        ...
        console.error(error);
        let errorMessage =
          'Error in getting the controller for group name "' +
          this.props.groupName +
          '"';
        this.setState({
          success: false,
  ...
  componentDidUpdate() {
    ...
    if (this.state.fixed_intensity) {
      waveform.setData(
        [this.state.fixed_intensity_val, this.state.fixed_intensity_val],
        [0, 10]
      );
    } else {
      if (this.state.schedule_values) {
        let values = this.state.schedule_values
            .split('\n')
            .map(val => parseFloat(val)),
  ...
  render() {
    var fixed_intensity_error =
      this.state.fixed_intensity_val > 0 &&
      this.state.fixed_intensity_val < 0.25;
      ...
      <div className="container-fluid" id="calib_light_list">
        ...
        <div className="col-sm-4">
          {!!this.state.controller && (
            <div>
              ...
              <div className="form-group">
                <div className="checkbox">
                  <label>
                    <input
                      type="checkbox"
                      name="fixed_intensity"
                      checked={this.state.fixed_intensity}
              ...
              {this.state.fixed_intensity ? (
                <div
                  className={
                    'form-group ' + (fixed_intensity_error ? 'has-error' : '')
                  }>
                  ...
                  <input
                    ...
                    max="1.5"
                    step="0.01"
                    name="fixed_intensity_val"
                    value={this.state.fixed_intensity_val}
                    onChange={this.handleInputChange}
                  />
                  {fixed_intensity_error ? (
                    <span className="help-block alert-danger">
                      The power supply cannot sustain light intensities below
                      0.25 sun. It will automatically turn off
                    </span>
                  ) : (
                    ''
                  )}
                ...
                <div>
                  <div className="form-group">
                    ...
                    <select
                      className="form-control"
                      name="schedule_basis"
                      value={this.state.schedule_basis}
                      onChange={this.handleInputChange}>
                  ...
                  <div className="form-group">
                    ...
                    <textarea
                      className="form-control"
                      name="schedule_values"
                      value={this.state.schedule_values}
                      onChange={this.handleInputChange}
                      onFocus={this.handleFocus}></textarea>
                    <span className="help-block">
                      Enter values (in suns) separated by new lines. Check the
                      graph on the right
                    </span>
                  </div>
                </div>
              ...
              {this.state.uv ? (
                this.state.uv.intensityMode == 'psuIntensity' ? (
                  <div
                    className={
                      'form-group ' + (fixed_intensity_error ? 'has-error' : '')
                    }>
                    ...
                    <div className="input-group">
                      <input
                        ...
                        max="100"
                        step="1"
                        name="fixed_uv_intensity_val"
                        value={this.state.fixed_uv_intensity_val}
                        onChange={this.handleInputChange}
                      />
                  ...
                  <div
                    ...
                    <div className="help-block">
                      Minimum value: 0 mW cm<sup>-2</sup>. Maximum value: 30 mW
                      cm<sup>-2</sup>
                    </div>
                  </div>
                )
          ...
          <div className="btn-group">
            <button className="btn-primary btn" onClick={this.apply}>
              Apply
            </button>
            <button className="btn-primary btn" onClick={this.save}>
              Validate
            </button>
            <button className="btn-default btn" onClick={this.close}>
              Close
            </button>
          </div>
        </div>
