...
class HTMLReportControl extends React.Component {
  ...
  savePDF() {
    ipcRenderer.send("htmlReport.savePDF", {
      cellName: this.props.cellInfo.cellName
    });
  }

  printPDF() {
    ipcRenderer.send("htmlReport.printPDF", {
      cellName: this.props.cellInfo.cellName
    });
  }
      ...
      influxquery(
        ...
        this.state.data.jv_available = values.map((value, index) => {
          return {
            ellapsed:
              Math.round(
                ((new Date(value[0]) - new Date(timefrom)) / 3600 / 1000) * 10
              ) / 10,
            time: value[0]
          };
        });
            ...
            <div className="col-sm-9">
              <select
                ...
                {this.state.data.jv_available.map(jv => (
                  <option key={jv.time} value={jv.time}>
                    {jv.ellapsed} hours
                  </option>
                ))}
              </select>
