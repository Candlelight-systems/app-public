...
class HTMLReport extends React.Component {
	...
	savePDF() {
      ...
      dialog.showSaveDialog( {
		...
		htmlToPDF.create( document.documentElement.outerHTML, { 
			format: 'A4', 
				...
				toReturn.jv = await influxquery( jvQuery, db, props.db ).then( ( results ) => {
					...
					return results.map( ( result, index ) => {
						...
						return {
							...
							waveform: waveform,
							waveInfo: getIVParameters( waveform, power, parseFloat( this.props.cellInfo.cellArea ), 100, true )
						}
					} );
		...
		return (
			...
			<div ref={ el => this.dom = el } className="container-fluid">
				...
				<div className="row">
...

						{
							!! this.state.data && !! this.state.data.jv && this.state.data.jv.map( ( jv, index ) => {
								...
								<div className="row ivData">
									...
									<div className={ "col-xs-2 color-series-style-" + index }></div>
					...
					<div className="col-xs-8" ref={ el => this.domStability = el }></div>
				</div>
