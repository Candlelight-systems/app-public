...
class statusIV extends GraphComponent {
  ...
  componentWillUpdate(nextProps) {
    ...
    if (shouldUpdateIV) {
      ...
      nextProps.data.forEach((data, index) => {
        if (
          data.time - lastInterval > idealInterval ||
          nextProps.data.length <= 5 ||
          index == nextProps.data.length - 1
        ) {
      ...
      this.graph
        .getYAxis()
        .setLowestMin(
          -environment.instrument[nextProps.instrumentId].fsr * 1e-3
        );
      this.graph
        .getYAxis()
        .setHighestMax(
      ...
      this.graph
        .getXAxis()
        .setLowestMin(
          -environment.instrument[nextProps.instrumentId].voltageRange
        );
      this.graph
        .getXAxis()
        .setHighestMax(
  ...
  componentDidUpdate() {
    this.ellipse.setPosition({
      x: this.props.voltage,
      y: this.props.current / 1000
    });
    this.ellipse.redraw();
  }
