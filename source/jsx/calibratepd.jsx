  ...
  async scalePD(sunValue) {
    ...
    if (!sunValue) {
      ...
      scalingFactor =
        sunValue /
        1000 /
        (this.state.channelsJsc['pd'] - this.state.darkOffset);
    }
    ...
    setTimeout(() => {
      ...
      fetch(
        `http://${this.props.config.trackerHost}:${
          this.props.config.trackerPort
        }/measureCurrent?instrumentId=${encodeURIComponent(
          this.props.instrumentId
        )}&groupName=${this.props.groupName}&chanIds=${str.join(',')}`,
        {
          method: 'GET'
        }
      )
    ...
    if (wrapper) {
      return (
        <span className="badge">
          ...
          {sunRatio
            ? ' / ' + Math.round(100 * val * sunRatio) / 100 + ' sun'
            : ''}
        </span>
      );
    }
    ...
    return (
      <div className="container-fluid" id="calib_light_list">
        <div className="col-sm-9">
          <div className="alert alert-info">
            ...
            <span
              ...
              aria-hidden="true"
            />{' '}
            Switch to manual mode in order to gain control of the light
            intensity
          </div>
        </div>
        {!!this.state.rescaling_success && (
          <div className="col-sm-9">
            <div className="alert alert-success">
              {' '}
              <span
                className="glyphicon glyphicon-check-sign"
                aria-hidden="true"
              />{' '}
          ...
          <ul className="list-group">
            ...
            {!!this.state.channels &&
              this.state.channels.map(channel => (
                <li
                  key={channel.chanId}
                  data-name={'mon_' + channel.chanId}
                  className={
                    'list-group-item ' +
                    (this.state['mon_' + channel.chanId] ? 'active' : '')
                  }
                  onClick={this.handleInputChangeLi}>
        ...
        <div className="col-sm-4">
          ...
          <p>
            To calibrate the light intensity with respect to the short circuit
            of your device, manually adjust the light intensity such that the
            short circuit current of the solar cells corresponds to the one
            measured on an AM1.5G source.
            <br />
            You can also reset the default settings to the factory calibrated
            reference photodiode short circuit current.
          </p>
          ...
          <div>
            <div className="form-group">
              ...
              <input
                ...
                readOnly="readonly"
                value={
                  (jsc = this.jsc('pd', false)).toPrecision(4)
                    ? jsc + ' mA'
                    : ''
                }
              />
              ...
              <div>
                ...
                <div className="btn-group">
                  <button
                    className="btn btn-default"
                    type="button"
                    onClick={() => this.scalePD(this.state.sunIntensity)}>
                    Calibrate
                  </button>
                  {!!this.state.control && (
                    <button
                      className="btn btn-default"
                      type="button"
                      onClick={() => this.scalePD()}>
                      Factory reset (
                      {Math.round(
                        100 *
                          this.state.channelsJsc['pd'] *
                          control.factory_scaling_ma_to_sun
                      ) / 100}{' '}
                      sun)
                    </button>
                  )}
