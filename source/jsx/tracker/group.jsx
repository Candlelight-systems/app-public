...
class TrackerGroupDevices extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      heatingPower: this.props.serverState
        ? this.props.serverState.heatingPower
        : 'N/A'
    });
  ...
  cfgAll() {
    ...
    ipcRenderer.once('channelsConfigured', (event, response) => {
      ...
      for (var i = 0; i < chanIds.length; i++) {
        data.chanStatuses[chanIds[i]] = Object.assign({}, response, {
          cellName: response['__cellName_' + chanIds[i]]
        });
  ...
  initCheckChannels(nextProps) {
    ...
    nextProps.channels.forEach(chan => {
      channelChecked[chan.chanId] =
        this.state.channelChecked[chan.chanId] || false;
    });
  ...
  doEnvironmentalSensing() {
    ...
    fetch(
      ...
      }/environmentalSensing?instrumentId=${
  ...
  resetSlave() {
    fetch(
      `http://${this.props.config.trackerHost}:${
        this.props.config.trackerPort
      }/resetSlave?instrumentId=${this.props.instrumentId}`,
      {
        method: 'GET'
      }
    ).then(response => {});
    ...
    if (this.props.channels) {
      var channelDoms = this.props.channels.map((chan, i) => {
        if (
          ...
          !this.props.serverState.channels ||
          !this.props.serverState.channels[chan.chanId]
        ) {
          return;
        }
      ...
      <div className="cl-group">
        ...
        <div className="row">
          <div className="cell-configure-all col-lg-9">
            <button className="btn btn-default btn-cl" onClick={this.checkAll}>
              <span className="glyphicon glyphicon-cog" />{' '}
              {this.state.checkAll ? 'Deselect all' : 'Select all'}
            </button>
            &nbsp;
            <button className="btn btn-default btn-cl" onClick={this.cfgAll}>
              <span className="glyphicon glyphicon-cog" /> Configure selected
            </button>
          </div>
        </div>
