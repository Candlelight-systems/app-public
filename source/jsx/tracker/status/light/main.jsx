...
class LightStatus extends React.Component {
  ...
  light_controller_setuv() {
    fetch(
      ...
      }/light.applyUV?instrumentId=${this.props.instrumentId}&groupName=${
        this.props.name
      }`,
      {
        method: 'GET'
  ...
  render() {
      ...
      case 'pyranometer':
        button_calibrate = (
          <button
            ...
            onClick={() => this.light_calibrate('calibratePyranometer')}>
            <span className="glyphicon glyphicon-scale" /> Calibrate pyranometer
          </button>
        );
      ...
      default:
        button_calibrate = (
          <button
            type="button"
            className="btn btn-cl btn-default btn-sm"
            onClick={() => this.light_calibrate('calibratePD')}>
            <span className="glyphicon glyphicon-scale" /> Calibrate photodiode
          </button>
        );
    ...
    if (
      this.state.lightUVSetpoint !== null &&
      !isNaN(this.state.lightUVSetpoint)
    ) {
      button_uv_set = (
        <button
          className="btn btn-cl btn-default btn-small"
          onClick={this.light_controller_setuv}>
          Apply UV intensity
        </button>
      );
    }
      ...
      <div className="group-status group-status-light col-lg-2">
        ...
        <div className="row">
          <div className="col-lg-9">
            {environment.statuses.light.version !== 'readonly' && (
              <button
                type="button"
                className="btn btn-cl btn-default btn-sm"
                onClick={this.light_controller_config}>
