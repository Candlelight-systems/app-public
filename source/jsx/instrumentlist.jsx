  ...
  constructor() {
    ...
    setInterval(() => {
      this.readConfig().instruments.forEach(cfg => {
        ...
        fetch('http://' + cfg.trackerHost + ':' + cfg.trackerPort + '/idn')
          .then(response => {
    ...
    ipcRenderer.on('instrumentUpdated', () => {
  ...
  render() {
    ...
    let instruments = config.instruments.map(config => {
      ...
      return (
        <li
          className={
            'list-group-item ' + (connected ? 'bg-success' : 'bg-danger')
          }
    ...
    instruments.push(
      <li className="list-group-item" onClick={this.addInstrument} key="__new">
        <a href="#">+ Add an instrument</a>
      </li>
    );

    return (
      <ul id="instrumentList" className="list-group">
        {instruments}
      </ul>
    );
  }
