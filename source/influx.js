...
export let query = function(query, db, cfg) {
  return new Promise((resolver, rejecter) => {
    ...
    $.get('http://' + cfg.host + ':' + cfg.port + '/query', params, function(
      results
    ) {
...
export let ping = cfg => {
  return new Promise((resolver, rejecter) => {
    fetch(address(cfg) + 'ping', { method: 'GET' }).then(data =>
      resolver(data)
    );
    setTimeout(() => rejecter('Query timeout'), 1000);
  });
};

export let checkAuth = async (cfg, u, p, db) => {
  const query_auth = `${address(cfg)}query?u=${u}&p=${p}&q=${encodeURIComponent(
    `SHOW GRANTS FOR "${u}"`
  )}`;
...
export let checkDB = async (cfg, u, p, db) => {
  ...
  const query_db = `${address(cfg)}query?u=${u}&p=${p}&q=${encodeURIComponent(
    `SHOW DATABASES`
  )}`;
